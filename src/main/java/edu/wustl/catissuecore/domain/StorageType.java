...
public class StorageType extends ContainerType implements ISpecimenTypeDomain {
	...
	public void setAllValues(IValueObject abstractForm) throws AssignDataException {
		try {
			...
			this.setTwoDimensionLabellingScheme(storageTypeForm.getTwoDimensionLabellingScheme());
			if (SearchUtil.isNullobject(this.capacity)) {
				this.capacity = new Capacity();
			}
			this.capacity.setOneDimensionCapacity(Integer.valueOf(storageTypeForm.getOneDimensionCapacity()));
			this.capacity.setTwoDimensionCapacity(Integer.valueOf(storageTypeForm.getTwoDimensionCapacity()));
		...
		if (storageTypeForm.getSpecimenOrArrayType().equals("Specimen")) {
			...
			if (specimenClassTypeArr != null) {
				...
				for (final String element : specimenClassTypeArr) {
					logger.debug("type Id :" + element);
					if (element.equals("-1")) {
						this.holdsSpecimenClassCollection.addAll(AppUtility.getSpecimenClassTypes());
						break;
					}
					else {
