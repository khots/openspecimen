 ...
 */
public class AddCRGAction extends CatissueAddEditAction {
	 ...
	 */
	@Override
	public ActionForward executeSecureAction(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		final String crgName = request.getParameter(Constants.CRG_NAME);
		final edu.wustl.catissuecore.bizlogic.CancerResearchBizLogic bizlogic = new CancerResearchBizLogic();
		String crgId = null;
		...
		if ((forward != null) && (forward.getName().equals(Constants.FAILURE))) {
			...
			try {
				...
				this.logger.error("Exception occurred in retrieving" + " Cancer Research Group" + bizExcep.getMessage(),
						bizExcep);
			}
		}
